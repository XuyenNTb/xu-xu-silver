...
namespace WebNhaHangOnline.Models
{
    public class DonhangKHModel
    {
        ...
        public void Luudonhang_(Donhangtongquan a, Giohang giohang, bool payOnline = false)
        {
            try
            {
                using (WebGiayHangHieuEntities db = new WebGiayHangHieuEntities())
                {
                    ...
                    dhkh.PhiVanChuyen = 0;
                    dhkh.MaAdmin = payOnline == true ? "1" : "0";
                    dhkh.thanhToanOnline = payOnline;
        ...
        public void Luudonhang(Donhangtongquan a, string maKH, Giohang giohang, Boolean? payOnline = false)
        {           
            try
            {
                using (WebGiayHangHieuEntities db = new WebGiayHangHieuEntities())
                    ...
                    if (tt == 2 || tt == 3)
                    {
                        foreach (var item in dh.ChiTietDonHangs)
                        {
                            ...
                            }else
                            {
                                ...
                                {
                                    string mail = email;
                                    string sub = "[Xác nhận email] Xác nhận đăng ký tại Shop";
                                    string bo = "";
                                    bo += "Xin chào " + hoten + ",<br>";
                                    bo += "Cảm ơn bạn đã đăng ký tịa Shop, Bạn sẽ được nhận những khuyến mãi của cửa hàng <br>";
                                    bo += "Xin cảm ơn.";
